			...
			try {
				...
				while (true) {
					...
					if (string.equals("CTS_Client_disconnect")) {// 当客户端窗口关闭时，触发的信号
						...
						//sb暂存向所有客户端发送某个客户端断开连接的提示
						sb.append("STC_Client_disconnect\n");
						sb.append(name + " disconnect\n\r");
					...
					}else if(string.equals("CTS_public_offline")){//离线被按下时
						
						String message=br.readLine();
						
						sb.append("STC_public_offline\n");//sb暂存向客户端发送的离线信号
						sb.append(message+"\n");//sb暂存消息
						
						sendMessageToAll(sb.toString());//发送至所有人
						jta.append(message+"\n");						
					}else if(string.equals("CTS_public_online")){//上线被按下时
						String message=br.readLine();
						
						sb.append("STC_public_online\n");//sb暂存向客户端发送的在线信号
						sb.append(message+"\n");//sb暂存消息
